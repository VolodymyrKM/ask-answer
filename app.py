...
def index():
    ...
    questions_cur = db.execute(
        """SELECT questions.id as question_id, questions.question_text,
...
def question(question_id):
    ...
    question_cur = db.execute("""SELECT  questions.question_text, questions.answer_text, askers.name as asker_name,
                                 experts.name as expert_name from questions join
                                 users as askers on askers.id = questions.asked_by_id join users as experts
                                 on experts.id = questions.expert_id where questions.id = ?""", (question_id,))
...
def answer(question_id):
    ...
    if request.method == "POST":
        ...
        where id = ?""", (request.form["answer"], question_id))
        db.commit()
...
def users():
    ...
    return render_template('users.html', user=user, user_results=users_results)
